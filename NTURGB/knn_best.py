...
# Create a KNN classifier and set the best k for testing
...
# ROC is not applicable for multiclass
# Calculate the parameters for the ROC curve
y_scores = knn_classifier.predict_proba(x_pca_test)[:,1]  # Get the predicted probabilities of the positive class
# Convert labels to binary labels
y_true_binary = [1 if label == 11 else 0 for label in y_test_10_11]
fpr, tpr, thresholds = roc_curve(y_true_binary, y_scores)  # Calculate the parameters for the ROC curve
# Plot the ROC curve
plt.figure()
...
plt.savefig('ROC of reading and writing')
plt.show()
...
plt.savefig('Confusion Matrix of reading and writing')
plt.show()
